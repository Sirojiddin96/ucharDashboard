...
-- Wrapped in DO block so it is skipped on fresh installs where public.services doesn't exist.
DO $$
BEGIN
  IF EXISTS (
...
-- Guarded: tariffs may not exist yet on a fresh project.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
...
-- ── Step 8: set_updated_at helper (idempotent) ───────────────
